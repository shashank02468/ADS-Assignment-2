...
def linePlot(df):
    ...
    Parameters
    ----------
    df : The dataframe used to plot line graph for plotting the CO2 emission
    from liquid fuel data

    Returns
    -------
    The function plots the line graphs over the years 2003-2015 for various
    countries, for the emission of CO2 from liquid fuel consumption
    '''
...
def histogram(filename):
    ...
    Parameters
    ----------
    df : The Dataframe parsed from main function which is extracted from world
    bank data for analyzing the climate changes.

    Returns
    -------
    The function doesn't return any value but it plots the histogram chart
    from the parsed dataframe'

    '''
    data = pd.read_csv(filename, skiprows=3)

    # Filter data for the United Kingdom and CO2 emissions 
    # from liquid fuel consumption
    data_uk = data[data['Country Name'] == 'United Kingdom']
    data_co2_liq = data_uk[data_uk["Indicator Name"] ==
                   'CO2 emissions from liquid fuel consumption (% of total)']
    data_co2_sol = data_uk[data_uk["Indicator Name"] ==
                   'CO2 emissions from solid fuel consumption (% of total)']
    data_co2_gas = data_uk[data_uk["Indicator Name"] ==
                   'CO2 emissions from gaseous fuel consumption (% of total)']

    # Select specific years
    selected_years = ["2003", "2004", "2005", "2006", "2007", "2008",
    ...
    plt.title('CO2 Emissions from Liquid Fuel Consumption \
in the United Kingdom (2003-2015)')
    plt.xlabel('Year')
    ...
    plt.title('CO2 Emissions from Solid Fuel Consumption in the United Kingdom\
(2003-2015)')
    plt.xlabel('Year')
    plt.ylabel('CO2 Emissions (kt)')
    plt.show()
    ...
    plt.title('CO2 Emissions from Gaseous Fuel Consumption in the \
...
def barchart(df):
    ...
    Parameters
    ----------
    df : The dataframe used to plotting bar graph for plotting the CO2 emission
    from liquid fuel data

    Returns
    -------
    The function plots the bar graphs over the years 2005-2015 for various
    countries, for the emission of CO2 from liquid fuel consumption
    '''
...
def readCSV(file):
    ...
    Parameters
    ----------
    file : The file is filename parsed from the main function for reading
    the database and analyzing the data.

    Returns
    -------
    This function reads the data from the provided filename and
    create two dataframes where one a coutries as columns and
    other as year as columns.
    '''
    ...
    countries = ["United Kingdom", "Italy", "Norway", "Finland",
                 "Germany", "Mexico"]

    # Filter the DataFrame for the indicator "CO2 emissions from 
    # liquid fuel consumption (kt)"
    df_CO2 = df[df["Indicator Name"] ==
                "CO2 emissions from liquid fuel consumption (kt)"]

    # Select specific columns for the years 2003 to 2015
    selected_years = ["2003", "2004", "2005", "2006", "2007", "2008", "2009",
...
def heatmap(file, country):
    ...
    indicators = ['EN.ATM.CO2E.KT', 'EN.ATM.CO2E.LF.KT', 'EN.ATM.CO2E.SF.ZS',
                  'EN.ATM.CO2E.GF.ZS']

    # Get the data from the World Bank dataset
    data = pd.read_csv(file, skiprows=(3))

    # Clean and format the data for the specified country 
    # and required indicators
    data = data.loc[data['Country Name'] == country]
    data = data.loc[data['Indicator Code'].isin(indicators)]
    data = data.loc[:, ["Indicator Name", "2003", "2004", "2005", "2006",
                        "2007", "2008", "2009", "2010", "2011", "2012",
                        "2013", "2014", "2015"]]

    # Drop non-numeric rows
    data = data.dropna(subset=["2003", "2004", "2005", "2006", "2007", "2008",
